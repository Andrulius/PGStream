...
void NetworkServer::sendStateUpdate(mg_connection* connection, const StreamStats& stats)
{
    ...
    json += ",\"pcmPacketDurationMs\":";
    json += juce::String(stats.pcmPacketDurationMs);
    json += ",\"pcmTargetBufferMs\":";
    json += juce::String(stats.pcmTargetBufferMs);
    json += ",\"pcmResumeBufferMs\":";
    json += juce::String(stats.pcmResumeBufferMs);
    json += ",\"pcmRingCapacityMs\":";
    json += juce::String(stats.pcmRingCapacityMs);
    json += ",\"transportMode\":";
